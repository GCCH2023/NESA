...
NesSubroutineRangeParser::NesSubroutineRangeParser(NesDataBase& db_) :
db(db_)
{

}

NesSubroutineRangeParser::~NesSubroutineRangeParser()
{
}

std::vector<SubroutineRange>& NesSubroutineRangeParser::Parse()
{
	Reset();
	// 依次识别3个中断处理程序
...
// 则认为是尾调用
void NesSubroutineRangeParser::ParseSubroutine(Address address)
{
...
void NesSubroutineRangeParser::AddSubroutine(Address start, Address end)
{
...
SubroutineRange* NesSubroutineRangeParser::GetSubroutineOrNext(Nes::Address address)
{
	// 查找第一个结束地址大于指定地址的子程序
...
SubroutineRange NesSubroutineRangeParser::ParseSubroutineRange(uint32_t start, uint32_t end)
{
...
void NesSubroutineRangeParser::Reset()
{
	subroutines.clear();
	subAddrs.clear();
}
