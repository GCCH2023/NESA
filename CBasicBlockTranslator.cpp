...
Expression* CBasicBlockTranslator::ConditionalJump(CNodeKind kind, const TAC* tac)
{
	// 条件跳转指令必定是基本块结束指令
	auto condition = GetNodeFactory().Binary(kind, GetExpression(tac->x), GetExpression(tac->y));
	auto jumpAddr = tac->z.GetValue();
	SetJumpCondition(condition);
	SetJumpTarget(jumpAddr);
	return condition;
}

Statement* CBasicBlockTranslator::TranslateTAC(const TAC* tac, size_t& index)
{
	...
	case TACOperator::CAST:
	{
		...
		expr = GetNodeFactory().Assign(result, expr);
	...
	case TACOperator::IFTRUE:
		expr = GetNodeFactory().Binary(CNodeKind::EXPR_NOT_EQUAL,
			GetExpression(tac->x), GetExpression(TACOperand(0)));
		SetJumpCondition(expr);
		SetJumpTarget(tac->z.GetValue());
		break;
	case TACOperator::IFFALSE:
		expr = GetNodeFactory().Binary(CNodeKind::EXPR_EQUAL, GetExpression(tac->x), GetExpression(TACOperand(0)));
		SetJumpCondition(expr);
		SetJumpTarget(tac->z.GetValue());
		break;
	...
	{
		// 先这样翻译凑合一下，翻译成表达式语句
		expr = GetNodeFactory().Binary(CNodeKind::EXPR_BAND, GetExpression(tac->x), GetExpression(tac->y));
		current = GetNodeFactory().ExprStat(expr);
		break;
	}
...
Statement* CBasicBlockTranslator::OnTranslate(const TACBasicBlock* block)
{
	CNode* expr = nullptr;
